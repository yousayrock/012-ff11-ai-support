...
`.env` を編集するだけで、キャラクターの名前・性格・口調を自由に変えられます。コードを触る必要はありません。
...
# プロフィール（種族・経歴・性格）
CHAR_PROFILE=エルヴァーン族の魔道士。クールで知識豊富。FF11歴15年のベテラン。

# 口調・話し方
CHAR_TONE=落ち着いた口調。「〜ですわ」「〜でしょう」。丁寧だが冷静。

# 口調の参考例
CHAR_TONE_EXAMPLES=「そのWSの散開タイミングは0.5秒前ですわ。」\n「装備は状況次第ですが、私ならこちらを選びますわ。」
```

キャラクター画像も `mia/` フォルダの PNG を差し替えるだけで変えられます（313×313px 透過PNG 推奨）。
